...
interface TranslatorFixtureTrace {
  sessions: { sourceLanguage: string; targetLanguage: string }[];
  calls: string[];
  destroyed: number;
  pending: boolean;
  release?: () => void;
...
/** Only the browser's native API is controlled; extension production code runs unchanged. */
async function installTranslatorFixture() {
  await panel.locator('body').evaluate(() => {
    const trace: TranslatorFixtureTrace = {
      sessions: [],
      calls: [],
      destroyed: 0,
      pending: false,
    };
    (window as Window & { __translatorFixture?: TranslatorFixtureTrace }).__translatorFixture =
      trace;
    Object.defineProperty(window, 'Translator', {
      configurable: true,
      value: {
        availability: async () => 'available',
        create: async (options: { sourceLanguage: string; targetLanguage: string }) => {
          trace.sessions.push({
          ...
          return {
            translate: async (text: string, options?: { signal?: AbortSignal }) => {
              trace.calls.push(text);
              // Hold cue 2 to observe incremental rendering and cancellation deterministically.
              if (trace.calls.length === 2) {
                ...
                await new Promise<void>((resolve, reject) => {
                  ...
                  const cancel = () => {
                    ...
                    signal?.removeEventListener('abort', cancel);
                    cancelPending = undefined;
                    reject(new DOMException('Test translation cancelled', 'AbortError'));
                  };
...
async function translatorTrace() {
  return panel.locator('body').evaluate(() => {
    const trace = (window as Window & { __translatorFixture?: TranslatorFixtureTrace })
      .__translatorFixture;
    if (!trace) throw new Error('Translator test fixture is missing');
    return {
...
async function releaseTranslation() {
  await panel.locator('body').evaluate(() => {
    (
      window as Window & { __translatorFixture?: TranslatorFixtureTrace }
    ).__translatorFixture?.release?.();
  });
}
...
test('independent learning page: source untouched, embedded playback, real API, summary, translation, Q&A and exports', async ({}, testInfo) => {
  ...
  const pdf = await printPage.pdf({ path: testInfo.outputPath('export.pdf'), format: 'A4' });
...
test('automatic translation without an API key uses the browser Translator incrementally and shows both subtitle panes', async ({}, testInfo) => {
  ...
  await expect(panel.locator('#test-connection')).toBeDisabled();
  // Earlier tests leave cached translations for this video; without clearing them the engine
  // switch below restores all four cues and the Translator fixture is never exercised.
  await panel.locator('#clear-cache').click();
  await panel.getByRole('button', { name: '关闭设置', exact: true }).click();
  await expect(panel.locator('#settings-dialog')).not.toBeVisible();
  await openSubtitleSettings();
  // Install before switching engines: on a machine whose Chrome has the language pack, the real
  // Translator would finish the whole transcript before the fixture could observe anything.
  await installTranslatorFixture();
  await panel.locator('#engine-google').click();
  // The engine is shown by the toggle and the hint; the button itself is a plain action.
  await expect(panel.locator('#engine-google')).toHaveAttribute('aria-selected', 'true');
  await expect(panel.locator('#translation-hint')).toContainText('Google');
  await expect(panel.locator('.translation')).toHaveCount(0);
  const beforeCalls = provider.calls.length;
  await panel.locator('#translate-btn').click();

  await expect.poll(async () => (await translatorTrace()).pending).toBe(true);
  await expect(panel.locator('.translation')).toHaveCount(1);
  await expect(panel.locator('.translation')).toContainText('Google 测试译文');
  await expect(panel.locator('#settings-dialog')).not.toBeVisible();
  ...
  await panel.locator('[data-cue="2"]').click();
  await expect(learning.locator('#original')).toHaveText('Check evidence before deciding.');
  await expect(learning.locator('#translated')).toHaveText(
    'Google 测试译文：Check evidence before deciding.',
  );
  const trace = await translatorTrace();
  expect(trace.sessions).toEqual([{ sourceLanguage: 'en', targetLanguage: 'zh' }]);
  expect(trace.calls).toEqual(CAPTIONS.events.map((cue) => cue.segs[0]!.utf8));
  expect(trace.destroyed).toBe(1);
  expect(provider.calls).toHaveLength(beforeCalls);
  await testInfo.attach('controlled-browser-translator-trace', {
    body: Buffer.from(JSON.stringify(trace, null, 2)),
    contentType: 'application/json',
  });
  await learning.screenshot({
    path: testInfo.outputPath('google-translation.png'),
    fullPage: true,
  });
  expect(browserErrors).toEqual([]);
});
...
test('cancelling browser translation destroys its session and does not translate later cues', async () => {
  ...
  await expect.poll(async () => (await translatorTrace()).destroyed).toBe(1);
  await releaseTranslation();
  const trace = await translatorTrace();
  expect(trace.pending).toBe(false);
...
test('missing browser translation reports an error without AI fallback; explicit AI without a key opens settings', async () => {
  ...
  await panel.locator('body').evaluate(() => {
    // This fixture represents browsers that do not expose Chrome's built-in Translator API.
    Object.defineProperty(window, 'Translator', { configurable: true, value: undefined });
  });
  const beforeCalls = provider.calls.length;
  await panel.locator('#translate-btn').click();
  await expect(panel.locator('#notice')).toContainText(/Chrome|浏览器/);
  await expect(panel.locator('#job-status')).not.toBeVisible();
  await expect(panel.locator('#settings-dialog')).not.toBeVisible();
  await expect(panel.locator('#translate-btn')).toBeEnabled();
  expect(provider.calls).toHaveLength(beforeCalls);
