...
> 已上架 Chrome 应用商店。默认**自带 Key**：扩展免费，字幕和提问只发给你选择的服务商。
> 也可以选择**登录订阅**：Google 账号一键登录，不用申请和填写 API Key，全部 AI 功能开箱即用，另享跨视频「我的资料库」，在看过的所有视频里按内容找到那一段。目前内测中，仅限受邀用户。
...
4. 需要 AI 总结或问答时，在顶部「AI 设置」选一种运行方式：
   - **用自己的 Key**（默认）：选 **服务商 → 模型**，填 **API Key**。扩展免费，按你在服务商那里的实际用量付费；默认在本机记住密钥，可在设置里取消勾选。
   - **登录订阅**：Google 账号一键登录，不用申请和填写 API Key，全部 AI 功能开箱即用。目前内测中，仅限受邀用户；未开通的账号会提示改用自己的 Key。
