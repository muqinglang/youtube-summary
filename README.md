...
Chrome / Edge Manifest V3 扩展，把 YouTube 视频变成可检索、可复习的学习笔记。当前版本 `0.8.2`。
...
> 已上架 Chrome 应用商店。用 Google 账号登录后使用：笔记、术语和总结都保存在你的账号里，换台电脑、重装扩展，打开同一个视频内容都还在。
> AI 功能使用**你自己的 Key**：扩展免费，字幕和提问只发给你选择的服务商。
...
1. 打开或刷新 YouTube 视频页，点击视频下方点赞左侧的 **旁听 AI** 按钮，或工具栏中的旁听图标。扩展会暂停原视频并打开独立学习标签；同一视频再次点击会回到已有标签。首次使用先点「用 Google 账号登录」，之后保持登录。
...
- 同步以最后修改的一方为准：同一个视频的笔记若在两台电脑上先后修改，以后改的那次为准，另一台上较早的修改不会合并进来。在同一台电脑上换另一个 Google 账号登录时，本机上属于上一个账号的副本会被清除，内容仍在上一个账号里。
- 订阅（托管模式）暂未开放。
