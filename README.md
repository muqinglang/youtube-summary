...
> 已上架 Chrome 应用商店。默认**自带 Key** 模式下扩展不依赖任何服务器，所有数据留在本机。
> 设置里也可切换到**登录订阅**模式：任务在[服务端](server/README.md)运行，不用自己配 Key，同一个视频别人处理过就直接复用，不重复计费；还多一个跨视频检索，在你看过的所有视频里按语义找片段。目前为内测，仅受邀账号可用。
...
4. 需要 AI 总结或问答时，在顶部「AI 设置」选一种运行方式：
   ...
   - **登录订阅**：用 Google 账号登录，任务在服务端运行，不用自己配 Key。命中共享缓存的视频不计费；未开通或用量用尽会提示改回自己的 Key。目前为内测，仅受邀账号可用。
